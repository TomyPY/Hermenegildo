...
function verCatalogoGenero(peliculas, genre) {
    let peliculasgenre = []
    let availableGenres = ['drama', 'sci-fi', 'horror', 'animation', 'fantasy', 'action']

    if (!availableGenres.some(item => genre.indexOf(item.toLowerCase()) >= 0)) {
        return undefined
    }

    peliculasgenre = peliculas.filter((pelicula) => {
        for (gen of pelicula.genre) {
            if (genre.includes(gen.toLowerCase())) {
                return (pelicula)
            }
        }
    })
...
function generateSlider(genre, peliculas) {
    ...
    const movieList = document.getElementsByClassName(`movie-list-${genre instanceof Array ? genre[0] : genre}`)[0]
    movieList.innerHTML = ""
    ...
    document.getElementsByClassName(`movie-list-container-${genre instanceof Array ? genre[0] : genre}`)[0].classList.remove("hide")
...
document.getElementsByClassName("fiction-panel")[0].addEventListener("click", () => {
    generateSlider(["fantasy", "sci-fi"], peliculas)
})
